...
const AddCategory = ({ fmlink, toggle }) => {
	...
	const { category, error } = Categories;
	...
	useEffect(() => {
		if (category && category !== null) {
			if (category.fmcontent) {
				const contentBlock = htmlToDraft(category.fmcontent);
			...
			setSelectedGroup({
				label: category.underFolder,
				value: category.underFolder,
			});
			setFormData({
				_id: category._id ? category._id : null,
				isLive: category.isLive ? category.isLive : false,
				fmtitle: category.fmtitle ? category.fmtitle : '',
				fmsubtitle: category.fmsubtitle ? category.fmsubtitle : '',
				fmlabel: category.fmlabel ? category.fmlabel : '',
				fmlink: category.fmlink ? category.fmlink : '',
				fmcontent: category.fmcontent ? category.fmcontent : '',
				fmposition: category.fmposition ? category.fmposition : 0,
				seotitle: category.seotitle ? category.seotitle : '',
				isMenu: category.isMenu ? category.isMenu : false,
				underFolder: category.underFolder ? category.underFolder : '',
				folderId: category.folderId ? category.folderId : 0,
				featuredimg: category.featuredimg
					? category.featuredimg
					: 'uploads/big-placeholder.jpg',
				extraboxes: category.extraboxes ? category.extraboxes : [],
			});
		} else {
			setFormData({
				isLive: false,
		...
		if (etitle !== '' || econtent !== '' || eposition !== 0) {
			...
			try {
				...
				dispatch(
					postActivity({
						logtype: 'positive',
						logcontent: `The category item <strong>${formData.fmtitle}</strong> was edited successfully`,
			...
			try {
				...
				dispatch(
					postActivity({
						logtype: 'positive',
						logcontent: `The category item <strong>${formData.fmtitle}</strong> was created`,
						email: user ? user.email : '',
					})
				);
				setFormData({
			...
			setShowError(true);
			setErrorMsg(`Title, Content and Folder are required`);
			setTimeout(() => {
				setShowError(false);
				setErrorMsg('');
			}, 7000);
