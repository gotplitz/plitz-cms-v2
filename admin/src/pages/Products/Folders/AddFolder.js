...
const AddFolder = ({ fmlink, toggle }) => {
	...
	const { folder, loading, error } = Folders;
	...
	useEffect(() => {
		if (folder && folder !== null) {
			if (folder.fmcontent) {
				const contentBlock = htmlToDraft(folder.fmcontent);
			...
			setFormData({
				_id: folder._id ? folder._id : null,
				isLive: folder.isLive ? folder.isLive : false,
				fmtitle: folder.fmtitle ? folder.fmtitle : '',
				fmsubtitle: folder.fmsubtitle ? folder.fmsubtitle : '',
				fmlabel: folder.fmlabel ? folder.fmlabel : '',
				fmlink: folder.fmlink ? folder.fmlink : '',
				fmcontent: folder.fmcontent ? folder.fmcontent : '',
				fmposition: folder.fmposition ? folder.fmposition : 0,
				title: folder.title ? folder.title : '',
				id: folder.id ? folder.id : 100,
				seotitle: folder.seotitle ? folder.seotitle : '',
				isMenu: folder.isMenu ? folder.isMenu : false,
				featuredimg: folder.featuredimg
					? folder.featuredimg
					: 'uploads/big-placeholder.jpg',
				extraboxes: folder.extraboxes ? folder.extraboxes : [],
			});
		} else {
			setFormData({
				isLive: false,
	...
	const onSubmit = (e) => {
		...
		if (etitle !== '' || econtent !== '' || eposition !== 0) {
			...
			try {
				dispatch(editFolder(formData));
				dispatch(
					postActivity({
						logtype: 'positive',
						logcontent: `The folder item <strong>${formData.fmtitle}</strong> was edited successfully`,
				...
				dispatch(
					postActivity({
						logtype: 'negative',
						logcontent: `An error occurred <em>editing</em> folder: <strong>${error}</strong>`,
						email: user ? user.email : '',
					})
				);
				setErrorMsg(error);
			...
			try {
				...
				dispatch(
					postActivity({
						logtype: 'positive',
						logcontent: `The folder item <strong>${formData.fmtitle}</strong> was created`,
						email: user ? user.email : '',
					})
				);
				setFormData({
				...
				setSelectedFiles([]);
				setSelectedFeatured([]);
				history.push('/all-folders');
			} catch (error) {
				dispatch(
					postActivity({
						logtype: 'negative',
						logcontent: `An error occurred adding folder: <strong>${error}</strong>`,
						email: user ? user.email : '',
					})
				);
			}
			...
			setShowError(true);
			setErrorMsg(
				`Title, Content and Folder are required, and Distec ID can't be 100`
			);
			setTimeout(() => {
				setShowError(false);
				setErrorMsg('');
					...
					<Col sm={6}>
						<div className='page-title-box'>
							...
							<ol className='breadcrumb mb-0'>
								...
								<li className='breadcrumb-item'>
									<Link to='/all-folders'>All Folders</Link>
								</li>
								<li className='breadcrumb-item'>Add Folder</li>
							</ol>
						</div>
					</Col>
				</Row>
