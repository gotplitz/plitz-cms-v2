...
const AddNoticia = ({ fmlink, toggle }) => {
	...
	const { noticia, loading, error } = Noticias;
	const Login = useSelector((state) => state.Login);
	const { user } = Login;

	useEffect(() => {
		if (noticia && noticia !== null) {
			if (noticia.fmcontent) {
				const contentBlock = convertFromHTML(noticia.fmcontent);
			...
			setFormData({
				_id: noticia._id ? noticia._id : null,
				isLive: noticia.isLive ? noticia.isLive : false,
				isTopNews: noticia.isTopNews ? noticia.isTopNews : false,
				fmtitle: noticia.fmtitle ? noticia.fmtitle : '',
				fmlink: noticia.fmlink ? noticia.fmlink : '',
				fmcontent: noticia.fmcontent ? noticia.fmcontent : '',
				seotitle: noticia.seotitle ? noticia.seotitle : '',
				fmcategory: noticia.fmcategory ? noticia.fmcategory : [],
	...
	const onSubmit = (e) => {
		...
		if (fmlink && fmlink !== '') {
			try {
				dispatch(editNoticia(formData));
				dispatch(
					postActivity({
						logtype: 'positive',
						logcontent: `The noticia item <strong>${formData.fmtitle}</strong> was edited successfully`,
				...
				dispatch(
					postActivity({
						logtype: 'negative',
						logcontent: `An error occurred <em>editing</em> noticia: <strong>${error}</strong>`,
						email: user ? user.email : '',
					})
				);
				setErrorMsg(error);
			...
			try {
				dispatch(addNoticia(formData));
				dispatch(
					postActivity({
						logtype: 'positive',
						logcontent: `The noticia <strong>${formData.fmtitle}</strong> was created`,
						email: user ? user.email : '',
					})
				);
				setFormData({
				...
				history.push('/all-noticias');
			} catch (error) {
				dispatch(
					postActivity({
						logtype: 'negative',
						logcontent: `An error occurred adding noticia: <strong>${error}</strong>`,
						email: user ? user.email : '',
					})
				);
			}
					...
					<Col sm={6}>
						<div className='page-title-box'>
							...
							<ol className='breadcrumb mb-0'>
								...
								<li className='breadcrumb-item'>
									<Link to='/all-noticias'>All News</Link>
								</li>
								<li className='breadcrumb-item'>Add News Item</li>
							</ol>
						</div>
					</Col>
				</Row>
							...
							<Row className='form-group'>
								<Col sm={12}>
									<input
										className='form-control'
										type='text'
										placeholder='Subtitle (a.k.a alt Title)'
										id='altTitle'
										name='altTitle'
										value={altTitle}
										onChange={(e) => onChange(e)}
						...
						<Col sm={12} md={3} className='pl-5'>
							<Card style={{ position: 'relative', zIndex: 1 }}>
								<CardBody>
									...
									<div className='mb-4'>
										<div className='mb-4 pb-4 setting-bottom-divider'>
											<p className='card-title-desc'>Is Featured</p>
											<Switch
												uncheckedIcon={<Offsymbol />}
												checkedIcon={<OnSymbol />}
												onColor='#6a479c'
